...
class FamilyMembers extends StatefulWidget {
  final String number;
  FamilyMembers({required this.number});
...
class _FamilyMembersState extends State<FamilyMembers> {
  Stream<QuerySnapshot>? FamilyStream;
  Methods md=Methods();
  ...
  getMembers()async{
    md.getFamily(widget.number).then((value){
      setState(() {
        FamilyStream=value;
      });
    });
        ...
        return snapshot.hasData ?ListView.builder(
            ...
            itemBuilder: (context,index){
              return messageTile(Name:snapshot.data?.docs[index]['Name'], MiddleName:snapshot.data?.docs[index]['MiddleName'],
                  ...
                  ,MainMobileNumber:widget.number);
            }
        ): Container();
      },
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Family',style: TextStyle(fontSize: 18,fontFamily: 'Poppins',color: Colors.white,fontWeight: FontWeight.bold),),
                  ...
                  child: Material(
                    ...
                    child: Container(
                      ...
                      child: MaterialButton(
                          ...
                          onPressed: () async{
                            FirebaseFirestore.instance.collection("users").doc("$MainMobileNumber").collection('Family').doc('$Name $MiddleName $SurName').delete()
                                .then((value) async{
